...
- **Gameplay entry:** `runCmd @0x4084c0`, two-pass `roundStartInit @0x40a4d0` (`levelSceneTexturesLoad @0x4104b0`, `levelSetup @0x4108a0`), `gameWorldUpdate @0x40b3d0`, `gameFrameRender @0x40ae30` / `gameRunFrame @0x40ad80` fixed-step handoff.
...
Zone data, `AR`/`IN` pairing, detail grid, FLOOR and NavBuoy loading plus Dijkstra pathfinding (`eload`/`nload`, 97 nav nodes verified on woodshop; `navPointRelaxCosts`/`PickCheapestLink`/`FindNearest*`/`PickRandom`/`ResetAllFlags` live) are live. Remaining gaps: per-level directors `levelEventDirector_L1..L4` and `roundTeardown @0x40aa10` / full `unloadGameWorld` resource-freeing tail; gameplay quest/sound directors; `g_apGrabbMesh @0x4583b8` population. The NavBuoy Dijkstra cluster currently has no live consumer (`aiPathfindToTarget` still routes via the FLOOR wall-avoidance path).

## Next steps

1. `levelEventDirector_L1..L4` + `_Init`/`_Cleanup` and `roundTeardown @0x40aa10` / `unloadGameWorld` resource-freeing tail.
2. Populate `g_apGrabbMesh @0x4583b8` from object loading and finalize `playerFindNearestTarget` owner checks.
3. Wire `aiPathfindToTarget` to the live NavBuoy graph (`navPointFindNearestInYRange` + `PickCheapestLink`) once the AI steering consumer is ready.
