...
- **Player round setup:** `playerSetupCharacters @0x41b6e0`, `playerSetupRound @0x410e90`, `playerSetupSceneObjects @0x411550`, `levelObjectsCartsCameraInit @0x411b70`, `cameraFollowUpdate @0x4020d0`. The follow camera builds its wall-avoid `GxVec2`s as x=slot[2], y=slot[0] (cdecl push order per `@0x402193`/`@0x4022d0`) and `zoneAvoidWalls @0x4023e0` skips pushes inside the `c_nc` blocker zone (`@0x44e214`, not `c_ac`), so the level-1 spawn camera sits at the original further-back distance.
...
Zone data, `AR`/`IN` pairing, detail grid, FLOOR and NavBuoy loading plus Dijkstra pathfinding (`eload`/`nload`, 97 nav nodes verified on woodshop; `navPointRelaxCosts`/`PickCheapestLink`/`FindNearest*`/`PickRandom`/`ResetAllFlags` live) are live. Remaining gaps: per-level directors `levelEventDirector_L2..L4` and `roundTeardown @0x40aa10` / full `unloadGameWorld` resource-freeing tail; gameplay quest/sound directors.

`levelEventDirector_L2..L4` + `_Init`/`_Cleanup` is temporarily out of scope as we focus on the first two levels, L0/L1.
