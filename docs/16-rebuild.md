...
The rebuild provides the original offline GUI flow through `DRIVERS\\GXSOFT.DLL`: the six-logo introduction leads to the five-row main menu, game-type selection, and character selection. Keyboard navigation, row dispatch, quit confirmation, clean shutdown, menu sound effects, and the animated, textured, lit 3D character preview are functional.

The character-selection scene uses the original `SEN`/`TPG` asset pipeline and scene-graph, animation, projection, material, and polygon-rendering paths. All ten character models can be selected and previewed.
...
- **Application and menu:** window/frame-loop handling, GX initialization, intro timeline, main menu, game-type selection, mode/player policy, character selection, quit confirmation, and keyboard-driven navigation.
